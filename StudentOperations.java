...
import java.sql.*; import java.util.Scanner; class StudentOperations {
    // Method to add a new student     public void addStudent(Scanner scan) {
        try (Connection conn = DBConnection.getConnection()) {
            System.out.print("Enter name: ");
            String name = scan.nextLine();
            System.out.print("Enter PRN: ");             long prn = scan.nextLong();             scan.nextLine();
            System.out.print("Enter Department: ");
            String dept = scan.nextLine();
            System.out.print("Enter Batch (YYYY-YY): ");
            String batch = scan.nextLine();
            System.out.print("Enter CGPA: ");             double cgpa = scan.nextDouble();             scan.nextLine();
            String sql = "INSERT INTO students (prn, name, dept, batch, cgpa) VALUES
(?, ?, ?, ?, ?)";
            PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setLong(1, prn);             stmt.setString(2, name);             stmt.setString(3, dept);             stmt.setString(4, batch);             stmt.setDouble(5, cgpa);             stmt.executeUpdate();
    ...
    // Method to search student by PRN     public void searchByPRN(Scanner scan) {         System.out.print("Enter PRN: ");         long prn = scan.nextLong();         scan.nextLine();
        try (Connection conn = DBConnection.getConnection()) {             String sql = "SELECT * FROM students WHERE prn = ?";             PreparedStatement stmt = conn.prepareStatement(sql);
            stmt.setLong(1, prn);             ResultSet rs = stmt.executeQuery();
            if (rs.next()) {
                System.out.println("Name: " + rs.getString("name") +
                        ", PRN: " + rs.getLong("prn") +
                        ", Dept: " + rs.getString("dept") +
    ...
    // Method to update student details     public void updateStudent(Scanner scan) {
        ...
        long prn = scan.nextLong();         scan.nextLine();
        try (Connection conn = DBConnection.getConnection()) {
            String checkSql = "SELECT * FROM students WHERE prn = ?";
            PreparedStatement checkStmt = conn.prepareStatement(checkSql);
            checkStmt.setLong(1, prn);             ResultSet rs = checkStmt.executeQuery();
            if (rs.next()) {
                ...
                String batch = scan.nextLine();
                System.out.print("Enter new CGPA: ");                 double cgpa = scan.nextDouble();                 scan.nextLine();
                String updateSql = "UPDATE students SET name=?, dept=?, batch=?, cgpa=? WHERE
prn=?";
                PreparedStatement stmt = conn.prepareStatement(updateSql);
                stmt.setString(1, name);                 stmt.setString(2, dept);                 stmt.setString(3, batch);                 stmt.setDouble(4, cgpa);                 stmt.setLong(5, prn);                 stmt.executeUpdate();
    ...
    // Method to delete student by PRN     public void deleteStudent(Scanner scan) {
        ...
        try (Connection conn = DBConnection.getConnection()) {
            ...
            stmt.setLong(1, prn);             int rows = stmt.executeUpdate();
            if (rows > 0) {
